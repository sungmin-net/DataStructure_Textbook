    ...
    if (maxElementCount > 0) {
        ...
        if (pReturn != NULL) {
            ...
            if (pReturn->pElement == NULL) {
                printf("오류, 2번째 메모리 할당, createArrayHeap()\n");
                free(pReturn);
                return NULL;
            }
            ...
            printf("오류, 메모리 할당, createArrayHeap()\n");
        }
    } else {
...
void insertArrayMaxHeapNode(ArrayHeap* pHeap, HeapNode element) {
    int i = 0;
    if (pHeap != NULL) {
...
HeapNode* extractArrayMaxHeapNode(ArrayHeap* pHeap) {
    ...
    if (pHeap != NULL && pHeap->currentElementCount > 0) {
        ...
        if (pReturn == NULL) {
            printf("오류, 메모리 할당, bringArrayMaxHeapNode()\n");
            return NULL;
        }
...
void insertArrayMinHeapNode(ArrayHeap* pHeap, HeapNode element) {
    int i = 0;
    if (pHeap != NULL) {
        if (pHeap->currentElementCount == pHeap->maxElementCount) {
            printf("Error, heap is full [%d], insertNodeArrayMinHeap()\n",
                    pHeap->maxElementCount);
            return;
        }
...
HeapNode* extractArrayMinHeapNode(ArrayHeap* pHeap) {
    HeapNode* pReturn = NULL;
    HeapNode* pTemp = NULL;
    ...
    if (pHeap != NULL && pHeap->currentElementCount > 0) {
        pReturn = (HeapNode*)malloc(sizeof(HeapNode));
        if (pReturn == NULL) {
            printf("Error, malloc(), extractArrayMinHeapNode(()\n");
            return NULL;
        }
        *pReturn = pHeap->pElement[ 1 ];
